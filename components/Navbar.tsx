            ...
            <nav className="hidden lg:flex items-center gap-1">
              {navLinks.map((link, i) => (
                <motion.button
                  ...
                  style={{
                    color: activeSection === link.href.slice(1) ? '#2563EB' : '#475569',
                  }}
                >
                  {link.label}
                  <span
                    ...
                    style={{
                      ...
                      width: activeSection === link.href.slice(1) ? '70%' : '0%',
                    }}
                  />
                  <span
                    className="absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] rounded-full opacity-0 group-hover:opacity-50 transition-all duration-300"
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <motion.span animate={{ rotate: mobileOpen ? 45 : 0, y: mobileOpen ? 8 : 0 }}
                  className="block w-6 h-0.5 origin-center" style={{ background: '#2563EB' }} />
                <motion.span animate={{ opacity: mobileOpen ? 0 : 1 }}
                  className="block w-4 h-0.5" style={{ background: '#2563EB' }} />
                <motion.span animate={{ rotate: mobileOpen ? -45 : 0, y: mobileOpen ? -8 : 0 }}
                  className="block w-6 h-0.5 origin-center" style={{ background: '#2563EB' }} />
              </button>
            </div>
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            ...
            <div
              className="w-72 h-full flex flex-col pt-20 pb-8 px-8 gap-2"
              style={{
            ...
            >
              ...
              {navLinks.map((link, i) => (
                <motion.button
                  ...
                  onClick={() => scrollTo(link.href)}
                  className="text-left py-3.5 px-4 rounded-xl text-slate-600 hover:text-blue-600 transition-colors font-medium border border-transparent hover:border-blue-100 hover:bg-blue-50"
                >
                  {link.label}
                </motion.button>
              ))}
              <div className="mt-auto space-y-4">
                <button onClick={() => scrollTo('#contact')} className="btn-primary w-full text-center text-sm">
                  Me contacter
                </button>
                {/* Socials mobile */}
